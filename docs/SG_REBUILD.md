...
```bash
cd backend && uv run python scripts/audit_sg.py      # the diagnosis of the retired tables
cd backend && uv run python scripts/fit_baselines.py # the cheap fix, and why it fails
cd backend && uv run python scripts/verify_sg.py     # the gates on the live path
```

---

## State of play — updated 2026-09-02

Sections below this one were written while nothing was wired into the live
path. That stopped being true in PR #20 (`feat: four-category strokes gained`),
which shipped phase 1's surface plus a hole-level attribution, so the plan's
"nothing wired" framing is history, not status. What is true now:

**Live:** the scorecard path serves all four categories from the
dispersion-derived surface (`surface_cache.py`, `scorecard_stats.py`).

**The `feature/sg-consistency-rework` branch then fixed what shipping
revealed.** The PR #20 attribution freelanced its baselines — a hard-coded
20-yard-rough stand-in for every missed green, a point-evaluated chip
proximity, average-drive arithmetic — and every shortcut was a bias. Its
docstring promised a 50/50 penalty split it never implemented, and its
`broadie.py` tables failed provenance review: SG_REBUILD said the needed data
exists only as chart images, the next commit contained precise tables
attributed to Broadie, and Sam confirmed he never transcribed them. Fabricated
data, deleted. Its bracket inconsistency was also a live crash — handicap
over 18 plus a GIR hole with no recorded first putt raised `ValueError`, and
the API swallowed it and silently dropped the whole round's stats row.

The rework replaces all of it with one rule: **every baseline the
reconstruction compares against is a conditional expectation under the same
generative model the surface is solved from** (`HoleConditionals`, solved by
`expected_strokes.solve_conditionals`, cached in `surfaces_v2.pkl`). That
buys, verified by `scripts/verify_sg.py` in CI:

- **Conservation, exact:** g+p+f+a = E_tee − score on every branch of every
  hole — the telescoping identity the PR #20 code claimed and broke on every
  missed green.
- **Totality:** no legal scorecard raises, handicaps 0–54, buckets or not.
- **Self-play zero-mean:** rounds generated by the model itself, entered as
  scorecards, attribute ≈ 0 in every category. PR #20's code read a
  bracket-average player as roughly G −2.5, F +3.0, A −9 at 25 handicap — it
  told nearly every user "approach is your leak, driving is fine" as an
  artifact of double-priced tee tails and penalties all landing on A.
- Fixing self-play exposed a real simulator bug that predates the branch: a
  ball landing greenside mid-iteration took a same-iteration full swing
  instead of a chip, and the fraction-of-distance dispersion makes a 15-yard
  "full swing" absurdly good, so simulated play beat the surface by over a
  stroke a round. One-shot-per-ball-per-iteration fixed it; calibration was
  re-run on the honest simulator.
- First-putt buckets are priced at their **certainty equivalent** under the
  model (separately for GIR arrivals and post-chip arrivals), not at a
  midpoint — midpoint reads mis-price the skewed within-band distribution,
  which showed up as a flat +0.4 putting bias.
- Penalties get no separate term: a scorecard cannot say which swing incurred
  them, the conditional baselines already price average penalty exposure, and
  charging them explicitly double-counted (the gate caught it as driving
  bias). Shot-level entry is what makes per-phase penalties honest.

**What is still open, in priority order:**

0. **A ±1-stroke P↔A transfer at 15–25 handicap, mirror-signed.** Self-play
   shows P +0.6…+0.8 against A −0.5…−1.1 there (exact first-putt pricing
   reproduces it, so it is not the buckets). Cause: the analytic
   reconstruction prices *expected* greenside disasters (failed bunker
   escapes) inside P's baseline while the *realized* extra strokes sit in
   A's swing count, and the one-chip-per-missed-green assumption pulls the
   other way on walk-on paths. The full per-hole sampler (phase 3 below)
   resolves both by construction; `verify_sg.py` holds a 1.25 regression
   ceiling on it meanwhile, strict 0.35 everywhere else.

1. **The level residual** — phase 1's known putting shortfall. Bracket-average
   scorecards still total −2 (scratch) to −9 (25 hcp) because the surface
   under-counts putts: the on-green landing distribution is unconstrained and
   the flat 10% lag model rarely three-putts. This is calibration data work
   (first-putt distance on GIR by handicap; published three-putt rates), not
   attribution work. `verify_sg.py` prints it as a non-gating report. The
   long-run answer is self-calibration from the app's own recorded first-putt
   buckets — real per-user data the app already collects.
2. **The detailed shot-by-shot path still runs the retired tables.**
   `main.py`'s per-shot ingestion uses `get_baseline_for_handicap` — the
   hand-typed baselines `audit_sg.py` fails 4/8 — and a flat putts-per-hole
   for putting SG. Swap it onto the surface (per-shot SG =
   E(before) − 1 − E(after); `get_category()` exists for the four-way split)
   and the two entry modes become one engine at different conditioning
   levels: a round entered both ways then agrees on totals by construction.
3. Handicaps above 25 are clamped to the 25 bracket — the aggregates end
   there. Extending honestly needs data, not extrapolation enthusiasm.
